...
from modules.model import resnet18
...
class SLRModel(nn.Module):
    def __init__(
            ...
            weight_norm=True, share_classifier=True, eventImage=True
    ):
    ...
    def masked_bn(self, inputs, e1,e2,len_x, batch):
        def pad(tensor, length):
            return torch.cat([tensor, tensor.new(length - tensor.size(0), *tensor.size()[1:]).zero_()])

        x = torch.cat([inputs[len_x[0] * idx:len_x[0] * idx + lgt] for idx, lgt in enumerate(len_x)])
        e1 = torch.cat([e1[len_x[0] * idx:len_x[0] * idx + lgt] for idx, lgt in enumerate(len_x)])
        e2 = torch.cat([e2[len_x[0] * idx:len_x[0] * idx + lgt] for idx, lgt in enumerate(len_x)])
        
        x = self.conv2d(x,e1,e2, x.shape[0] // batch)
    ...
    def forward(self, x, len_x, label=None, label_lgt=None, e1=None, e2=None):
        if len(x.shape) == 5:
            # videos
            batch, temp, channel, height, width = x.shape
            x = x.reshape(batch * temp, channel, height, width)
            
            e1 = e1.reshape(batch * temp, channel, height, width)
            e2 = e2.reshape(batch * temp, channel, height, width)

            w = self.w
            weight = w / (torch.sum(w,dim=0) + self.epsilon)
            framewise = self.masked_bn(x, e1,e2,len_x, batch)
