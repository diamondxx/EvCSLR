...
class Processor():
    def __init__(self, arg):
        ...
        if os.path.exists(self.arg.work_dir):
            raise ValueError("Work Dir already exists!")
        else:
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.optimizer_args['start_epoch'], self.arg.num_epoch):
                ...
                if dev_wer < best_dev and test_wer < best_test:
                    best_dev = dev_wer
                    best_test = test_wer
                    
                    best_epoch = epoch
                    model_path = "{}_best_model_dev_{}_test_{}.pt".format(self.arg.work_dir,best_dev,best_test)
                    self.save_model(epoch, model_path)
                    self.recoder.print_log('Save best model')
                self.recoder.print_log('Best_dev: {:05.2f}, Best_test:{:05.2f}, Epoch : {}'.format(best_dev, best_test,best_epoch))

                # save last model
                self.save_model(epoch, self.arg.work_dir + 'last_model.pt')
    ...
    def load_data(self):
        ...
        if self.arg.dataset == 'CSL':
            ...
            dataset_list = zip(["train", "train_eval", "dev", "test"], [True, False, False, False])
        elif self.arg.dataset == 'EvCSLRDataset':
