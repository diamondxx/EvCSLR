...
class Compose(object):
    ...
    def __call__(self, image, label, file_info=None,RD=None,w1=None,h1=None,index=None):
        for t in self.transforms:
            if file_info is not None and isinstance(t, WERAugment):
                image, label = t(image, label, file_info)
            else:
                image = t(image,RD,w1,h1,index)
        return image, label
...
class ToTensor(object):
    def __call__(self, video,RD,w1,h1,index):
...
class RandomCrop(object):
    ...
    def __call__(self, clip,RD,w1,h1,index):
